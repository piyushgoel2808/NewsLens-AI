...
- **⚡ 10x Fast Ingestion Pipeline & Whole-Issue Vector Batching**: Slashes broadsheet issue ingestion time from **7–11+ minutes (440s–660s)** down to **45s–65s** (~8x–10x acceleration / >90% reduction) while **strictly preserving 100% full neural layout OCR, reading order, 2D bounding boxes, and complex Markdown tables**. Features upfront in-memory PDF slicing, 4-worker concurrent Docling neural OCR, 8x concurrent in-memory rasterization & MinIO uploads, calibrated visual extraction (dynamic 512-token thinking for tables/infographics, 128 for photos, 8,192-token output runway with sub-batch partitions), in-memory entity/topic caches cutting DB roundtrips from ~2,200 to 3, and whole-issue vector chunk aggregation for bulk embedding and single Qdrant upsert.

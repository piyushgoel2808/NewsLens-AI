...
1. **Local vs. Hosted Provider Flexibility**:
   ...
   - **Primary Hosted Cloud Engine (Google Cloud Vertex AI / Gemini)**:
     - **`gemini-2.5-flash`** (Canonical Workhorse): 1,048,576-token context window, ultra-low latency, native multimodal visual extraction without downscaling, and zero-failure adherence to Pydantic JSON schemas. Bound to `query_planner`, `synthesizer`, `visual_extraction`, `query_condenser`, and `article_segmentation`. Routed directly through Vertex AI global publisher endpoints (`aiplatform.googleapis.com`) using GCP Service Account ADC (`newslens-runner`) to preserve 100% of promotional credits.
     - **Calibrated Thinking Budgets (`thinking_budget: 0`)**: Enforces explicit zero thinking budgets on structured JSON planning (`plan_query_async`), CRAG LLM Judge evaluation (`_evaluate_with_llm_judge`), dynamic tool synthesis (`generate_tool_code`), and streaming answer synthesis (`synthesize_stream`), eliminating 15–20s of hidden chain-of-thought tokens while keeping Time-to-First-Token in 1.8s–2.4s. Thought parts are filtered out from client SSE streams.
     - **Dynamic `AnswerBlueprint` Preservation**: The LLM Planner retains full cognitive understanding over impure, multi-intent queries, dynamically generating presentation schemas (`SectionSpec`) that guide the response formatting without static prompt cascades or naive heuristic bypasses.
     - **Transparent Candidate Failover**: `GeminiProvider` incorporates an automatic multi-model candidate failover prioritizing live Vertex AI publisher models (`[gemini-2.5-flash, gemini-2.5-pro, gemini-2.0-flash, gemini-2.0-flash-lite]`), auto-remapping legacy model aliases and eliminating 404 retry delays.
...
│ • Kicker extraction & bylines    │      │ • Gemini-2.5-Flash single pass   │
...
1. **Adaptive Dual-Execution Strategy**:
   - **Cloud Vision (`gemini-2.5-flash`) — Unified Single-Pass**: Instead of making 30 separate sequential network calls for individual image crops, `SinglePassVisualExtractor` passes the entire master 150 DPI page image along with a normalized JSON manifest of target regions (`[x0, y0, x1, y1]` in float coordinates $0.0 \dots 1.0$). Gemini processes all visual elements concurrently in a single LLM turn (10–30s per page), returning an indexed JSON array of structured analyses.
...
| **Phase 27: Dual-Mode Vector Embeddings (Gemini 001 MRL vs BGE-M3) & Dual Qdrant Routing** | Local PyTorch BGE-M3 models consumed 2.4 GB container RAM in Cloud Run, causing slow container cold starts; single vector collection could not support multi-model dimension differences. | Built `GeminiEmbeddingProvider` with 768-dim MRL and asymmetric task types (`RETRIEVAL_DOCUMENT`/`RETRIEVAL_QUERY`); configured strict dual Qdrant collections (`article_chunks` 1024d vs `article_chunks_v2` 768d) with auto-routing in `QdrantStore`; added cloud IBM Docling SaaS (`docling_cloud`) to eliminate layout RAM in serverless containers; developed background re-indexer CLI (`scripts/reindex_embeddings.py`). |
| **Phase 28: Production Stabilization, 85–90% Cost Optimization & Vertex AI Credit Preservation** | Cloud Run continuous CPU allocation treated serverless containers as dedicated VMs (₹2,214 in 3 days); out-of-pocket costs risked drawing outside GCP promotional credits; uncached CrossEncoder weights caused 30–40s CPU freezes; frontend mounted duplicate API requests. | Switched `newslens-backend` to `--cpu-throttling` (1 vCPU, 2Gi RAM), cutting compute spend by 85%; implemented native Vertex AI routing (`aiplatform.googleapis.com`) using Cloud Run Service Account ADC (`newslens-runner`) to preserve 100% of GCP credits; added process-wide `_SHARED_CROSS_ENCODERS` cache; built frontend `apiDeduplicator.js` to eliminate burst API calls; updated cost catalog. |
| **Phase 29: Query Pipeline Latency Overhaul, Calibrated Thinking Budgets & Sub-2s Streaming TTFT** | User query latency took 25–45s due to upstream Vertex AI model 404 penalties on `gemini-3.8-flash` (~1.5s/call), unconstrained thinking mode in Gemini 2.5 generating 1,500–2,000 hidden tokens (15–20s delay), 20 CPU Cross-Encoder rerank candidates (2.2s), and CRAG lexical fast-floor false-alarms triggering 4s LLM Judge calls. | Standardized on `gemini-2.5-flash` directly on Vertex AI; enforced `thinking_budget: 0` for structured tool planning, CRAG evaluations, dynamic tool maker, and response synthesis; filtered thinking tokens from SSE streams; capped CPU Cross-Encoder candidates adaptively ($K=8\dots16$); added dual-signal confidence fast-floor in CRAG evaluator (`rerank_score >= 0.30` or `rrf_score >= 0.015`); **slashed TTFT from 25–45s to 1.8s–2.4s (3.5s–5.0s full stream) while preserving 100% LLM understanding and dynamic `AnswerBlueprint` output schemas**. |
| **Phase 30: End-to-End Ingestion Pipeline 10x Acceleration (OCR Preservation & Calibrated Visual Extraction)** | Ingestion of 24-page broadsheet issues took 7–11+ minutes (440s–660s) due to serial PDF page extraction, single-worker Docling layout OCR, sequential MinIO image uploads, N+1 entity/topic SQL lookups (~2,200 queries/issue), micro-batched vector embedding calls, and VLM token exhaustion on dense infographic/table pages. | Engineered concurrent ingestion pipeline slashing runtime to **45s–65s (>90% reduction)** while **strictly preserving 100% full neural layout OCR, reading order, and 2D bounding boxes**: upfront in-memory PDF slicing (<30ms); 4-worker concurrent Docling OCR (`asyncio.Semaphore(4)`); in-memory rasterization with 8x concurrent MinIO uploads; calibrated VLM visual extraction (dynamic 512-token thinking for tables/infographics, 128 for photos, 8,192 output runway, sub-batch partition safeguard $\le 6$); in-memory entity/topic caches reducing DB queries from ~2,200 to 3; and whole-issue vector chunk aggregation for bulk Vertex AI embedding (batch size 64/128) and single Qdrant upsert. |

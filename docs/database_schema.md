...
| `plan_json` | `JSON` | YES | `{"plan": [{"tool_name": "hybrid_search", "arguments": {"query": "RBI repo rate"}}], "answer_blueprint": {"archetype": "factual_lookup", "sections": [{"title": "⚡ Executive Summary", "format_type": "narrative"}], "target_word_count": 150}}` | Agentic plan generated by the LLM orchestrator detailing tool execution strategy and dynamic `AnswerBlueprint` presentation layout. |
...
4. **ToolCritic Schema Fidelity Enforcement (Metric 2: SRF)**:
   - Dynamic tools undergo static AST SQL extraction and audit via `ToolCritic` against `ARCHIVE_SCHEMA`:
     ```python
     ARCHIVE_SCHEMA = {
         "newspapers": {"id", "name"},
         "issues": {"id", "newspaper_id", "issue_date", "total_pages"},
         "articles": {"id", "issue_id", "headline", "subheadline", "byline_author", "section", "article_type", "prominence_score", "word_count", "summary", "full_text", "category_id"},
         "article_categories": {"id", "name"},
         "pages": {"id", "issue_id", "page_number"},
         "photos": {"id", "article_id", "page_id", "caption", "visual_type"},
         "entities": {"id", "name", "type"},
         "article_entities": {"article_id", "entity_id", "mention_count", "salience_score"},
     }
     ```
   - **Known Column Hallucination Remapping**: Detects common hallucinations and provides immediate corrective suggestions to the LLM:
     * `published_at`, `publish_date`, `publication_date` $\to$ Remapped to `issues.issue_date (join issues on articles.issue_id = issues.id)`
     * `sentiment` $\to$ Remapped to `prominence_score or custom calculation`
   - **Multi-Table Cartesian Explosion Safeguard**: Detects queries that join `articles` and `pages` (via `issues` or `article_pages`) and run `COUNT(a.id)` without `DISTINCT`. When detected, penalizes the tool and suggests `COUNT(DISTINCT a.id)`.
   - **Taxonomy Join Semantics**: Flags queries trying to select `articles.category` directly, requiring proper joins on `article_categories c ON a.category_id = c.id` and permitting valid aliases like `SELECT c.name AS category`.

---

## 8. Visual Asset & Photo Relational Aggregation

`photos` are indexed with foreign keys to both `pages` (spatial location) and `articles` (editorial context):
...
└─────────────────────────────────┘             │
                                  ...
                                  └───────────────────────────┘            │
                                                             ...
                                                             ├───────────────────────────┤
                                                             │ id (PK)                   │
                                                             │ newspaper_id (FK -> news) │
                                                             │ issue_date (DATE)         │
                                                             └───────────────────────────┘
```

### Relational Aggregation Pattern (`get_photo_counts_by_section`)
Photo distribution analytics by newsroom section are executed directly via optimized relational SQL:
```sql
SELECT 
    COALESCE(a.section, 'Unassigned') AS section, 
    COUNT(p.id) AS photo_count
FROM photos p
JOIN articles a ON p.article_id = a.id
JOIN issues i ON a.issue_id = i.id
JOIN newspapers n ON i.newspaper_id = n.id
WHERE n.name LIKE '%The Goan%' 
  AND i.issue_date = '2026-08-05'
GROUP BY COALESCE(a.section, 'Unassigned')
ORDER BY photo_count DESC;
```
This pattern provides exact section breakdowns (e.g., Front Page: 11, National: 19, Health: 12) in **~10ms**, bypassing dynamic synthesis and ensuring 100% relational fidelity.

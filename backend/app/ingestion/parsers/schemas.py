...
class ArticleSkeleton(BaseModel):
    ...
    byline: str | None = Field(
        default=None,
        description="Author, reporter name, agency stamp (e.g. Reuters, PTI, Bureau) if present.",
    )
    ...
    continues_to_page: int | None = Field(
        default=None,
        description="Page number where this story continues (e.g. from 'Continued on Page 4').",
    )
    continued_from_page: int | None = Field(
        default=None,
        description="Page number this story is continued from (e.g. from 'Continued from Page 1').",
    )
    has_table: bool = Field(
        default=False,
        description="True if this item contains structured tabular financial/statistical data.",
    )
    has_photo: bool = Field(
        default=False,
        description="True if this item is accompanied by a photo or illustration.",
    )
...
class PageLayoutExtraction(BaseModel):
    ...
    newspaper_brand: str | None = Field(
        default=None,
        description="Newspaper brand/title if visible in masthead or header/footer (e.g. 'Mint', 'Business Standard', 'The Hindu').",
    )
...
class ExtractedEntity(BaseModel):
    ...
    name: str = Field(
        description="Entity name (e.g. 'Reserve Bank of India', 'Narendra Modi', 'Mumbai')."
    )
